...
public class JooqApplication {
	...
	CommandLineRunner demo(CustomerRepository customerRepository) {
		return args -> {
			customerRepository.selectAllCustomers().forEach(System.out::println);
		};
	}
...
class CustomerRepository {
	...
	public Collection<CustomerDTO> selectAllCustomers() {
		...
		Map<Record, Result<Record>> recordResultMap = this.dslContext.select().from(Customer.CUSTOMER)
				.leftJoin(Product.PRODUCT).on(Customer.CUSTOMER.ID.eq(Product.PRODUCT.CUSTOMER_ID)).fetch()
				.intoGroups(Customer.CUSTOMER.fields());
		return recordResultMap.values().stream().map(r -> {
			Record2<Integer, String> record2 = r.into(Customer.CUSTOMER.ID, Customer.CUSTOMER.EMAIL2).get(0);
			Integer customerId = record2.value1();
			String email = record2.value2();
			List<ProductDTO> productDTOS = r.sortAsc(Customer.CUSTOMER.ID).into(ProductDTO.class).stream()
					.filter(pdto -> pdto.getId() != null).collect(Collectors.toList());
			return new CustomerDTO(customerId, email, productDTOS);
		}).collect(Collectors.toList());
	}

	public void insertCustomer(String email) {
		this.dslContext.insertInto(Customer.CUSTOMER).columns(Customer.CUSTOMER.EMAIL2).values(email);
	}
